...
var Unshorteners = {
    ...
    generic: function (url, callback) {
        var cookie_jar = request.jar();
        var headers = (url.host === "t.co")?{}:{'User-Agent': userAgents[Math.floor(Math.random() * userAgents.length)] + Math.random().toString().substr(0,3), 'Referer': 'https://twitter.com/search?src=typd&q=' + url.hostname};
        var lastHeaderLoc = '';
        var triedFullGet = 0;
