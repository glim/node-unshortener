...
var options = {};
var memclient;
var userAgents = ['Mozilla/5.0 (Windows NT 6.2; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1667.0 Safari/537.36','Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; WOW64; Trident/6.0)','Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:24.0) Gecko/20100101 Firefox/24.0','Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_3) AppleWebKit/534.55.3 (KHTML, like Gecko) Version/5.1.3 Safari/534.53.10']
var map = {
    bitly: {
...
exports.config = function(opts) {
   ...
   if (typeof(opts.timeout) !== 'undefined') {
        request.defaults({'timeout':opts.timeout});
   }
   if (typeof(opts.userAgents) !== 'undefined') {
...
var Unshorteners = {
    ...
    generic: function (url, callback) {
        var cookie_jar = request.jar();
        var headers = (url.host === "t.co")?{}:{'User-Agent': userAgents[Math.floor(Math.random() * userAgents.length)] + Math.random().toString().substr(0,3), 'Referer': 'https://twitter.com/search?src=typd&q=' + url.hostname, 'Accept-language':'en-US', Accept: '*/*'};
        var lastHeaderLoc = '';
        var triedFullGet = 0;
        var handle = function (err, res) {
            if ( !err && res.statusCode && res.statusCode >= 200 && res.statusCode < 300 ) {
                ...
                if (!err) {
                    ...
                    if ((triedFullGet++ < 2) && (res.statusCode === 403 || res.statusCode === 405 || res.statusCode === 500 || res.statusCode === 501 || res.statusCode === 503)) {
                        if (triedFullGet == 2) {
                            delete headers['User-Agent'];
                        }
                        return request.get({ url:url, headers:headers, jar: cookie_jar, followRedirect: function(resp){ lastHeaderLoc = resp.headers.location; return true;} }, handle);
                    }
                }
                else {
                    if (err.code === 'HPE_INVALID_CONSTANT') {
                        url = urllib.parse(lastHeaderLoc);
                        err = null;
                    }
                }
                callback(err, url);
        ...
        request.head({ url:url, headers:headers, jar: cookie_jar }, handle);
    }
